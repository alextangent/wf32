Require time.fs                 \ Timing routines now in utils.f
...
2variable timevar                       \ a place to put 64-bit results
variable msecticks                      \ tick rez, usually CPU clock in kHz

1 import: QueryPerformanceFrequency
1 import: QueryPerformanceCounter
: InitCounter   ( -- )
   timevar QueryPerformanceFrequency drop
   timevar 2@ swap 1000 um/mod nip msecticks ! ; 

InitCounter 

\ Note: UM/MOD may crash when the time goes beyond 2^32 msec (49 days)

: COUNTER       ( -- ms )               \ time since CPU started
   timevar QueryPerformanceCounter drop 
   timevar 2@ swap msecticks @ um/mod nip ; 
